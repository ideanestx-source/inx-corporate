...
export const industryPages: IndustryPage[] = [
  // ─── SaaS Development ────────────────────────────────────────────────────────
  {
    ...
    capabilities: [
      ...
      "Authentication and authorization (SSO, SAML, OAuth 2.0, RBAC)",
    ...
    faqs: [
      ...
      },
      {
        ...
        answer:
          "The functional difference is multi-tenancy, billing infrastructure, and operational complexity. A standard web application serves one organization. A SaaS product serves many simultaneously, each with isolated data, configurable permissions, and independent billing. The architectural decisions required to make this work reliably — data isolation strategy, billing abstraction, API rate limiting, tenant-level observability — are different in character from standard web application engineering. Teams that approach SaaS development as web development with subscription payments added consistently underestimate the architectural work involved.",
      },
      {
  ...
  // ─── Healthcare Software Development ────────────────────────────────────────
  {
    ...
    heroSubtext:
      "Engineering clinical and operational software for healthcare organizations that cannot tolerate failure. From EHR integration to patient-facing applications, INX builds healthcare systems that meet regulatory requirements and perform in production clinical environments.",
    sections: [
      {
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "Beyond HIPAA, healthcare software must increasingly address state-specific privacy regulations, international data protection requirements for organizations with global patient populations, and emerging requirements around AI-generated clinical content. INX builds compliance infrastructure that is maintainable as the regulatory environment evolves, not just compliant at the point of initial delivery.",
        ],
      },
      {
        heading: "Integration with Clinical and Operational Systems",
        paragraphs: [
          "Healthcare software rarely operates in isolation. Patient data originates in EHR systems. Insurance claims move through clearinghouses. Laboratory results arrive via HL7 interfaces. Appointment data synchronizes with practice management systems. The integration landscape of a healthcare organization is typically a combination of modern APIs and legacy interfaces that predate REST by decades. Building software that integrates reliably with this environment requires understanding the standards, the variations in how those standards are implemented, and the operational patterns of the systems involved.",
          ...
          "INX builds healthcare integrations with explicit handling for the data quality variations and message format inconsistencies that occur in production environments. This means designing ingestion pipelines that validate and normalize incoming data, produce structured error records for messages that cannot be processed automatically, and provide operational tooling for resolving integration failures without engineering intervention.",
        ],
      },
      {
    ...
    faqs: [
      ...
      },
      {
        ...
        answer:
          "HIPAA establishes the floor for protected health information handling. INX designs healthcare systems to exceed that floor where the risk profile of the application warrants it. This includes zero-trust network architecture for systems handling sensitive clinical data, penetration testing as part of the delivery process, threat modeling during the architecture phase, and security incident response procedures documented before the system goes live. State-specific privacy regulations and international data protection requirements are addressed where they apply to the patient population the system serves.",
      },
      {
        question:
  ...
  // ─── FinTech Software Development ───────────────────────────────────────────
  {
    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "The cost of retrofitting compliance into a non-compliant financial system is consistently underestimated. A payment system that was built without cardholder data scope minimization will require architectural changes — not configuration changes — to achieve PCI DSS compliance. A KYC workflow that was bolted onto an existing user management system will have data model inconsistencies that create compliance gaps. These are not problems that can be resolved through audit preparation. They require engineering work on the underlying system.",
          "INX scopes compliance requirements during the architecture phase and designs systems that are compliant by construction rather than by retrospective remediation. This means working with the organization's compliance function during architecture — not after delivery — to ensure that regulatory requirements are fully understood before they are encoded in the data model and the application logic.",
        ],
      },
      {
        heading: "Payment Processing and Financial Data Infrastructure",
        paragraphs: [
          "Payment processing infrastructure is more complex than payment gateway integration. A gateway integration handles the payment authorization. The payment processing system handles the business logic that surrounds it: payment method management, retry logic for failed authorizations, partial payment handling, refund and dispute management, multi-currency support, and the reconciliation of gateway transaction records against internal ledger state. Each of these components has correctness requirements that must be designed and tested explicitly.",
      ...
      },
      {
        ...
        paragraphs: [
          "Financial systems are high-value targets for both external attackers and internal fraud. The security architecture of a financial system must address both threat categories. External attack surface reduction requires strict cardholder data scope minimization, network segmentation, and application-layer controls that prevent injection, authentication bypass, and data exfiltration. Internal fraud prevention requires transaction monitoring, anomaly detection, and segregation of duties in financial operations workflows.",
    ...
    capabilities: [
      ...
      "Card program and issuing infrastructure",
    ],
    faqs: [
      {
        question: "Does INX build PCI DSS compliant payment systems?",
        answer:
          "Yes. PCI DSS compliance is scoped during the architecture phase, not audited after delivery. This means designing cardholder data flows to minimize scope, implementing tokenization where applicable to reduce the volume of cardholder data the system handles directly, and documenting the compliance architecture in a format suitable for a Qualified Security Assessor review. INX works with the organization's compliance function throughout the engagement rather than producing a system and then attempting to make it compliant retrospectively.",
      },
      {
      ...
      },
      {
        ...
        answer:
          "Fintech engagements follow a structured discovery phase before development begins. The discovery phase covers compliance requirements, integration scope, correctness requirements for financial logic, and security threat modeling. This phase produces an architecture specification that the development team builds against, and a compliance documentation package that supports regulatory review. Development proceeds in phases that prioritize the foundational financial infrastructure — ledger, transaction processing, reconciliation — before building the application features that depend on it.",
      },
    ],
    relatedArticleSlugs: [
      "why-operational-context-matters",
  ...
  // ─── eCommerce Development ──────────────────────────────────────────────────
  {
    ...
    capabilities: [
      ...
      "High-performance product catalog and search",
      ...
      "Shopify and commercetools customization and extension",
      "Customer account and loyalty program systems",
    ],
    faqs: [
      {
        question:
  ...
  // ─── Gaming Software Development ────────────────────────────────────────────
  {
    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          "Multiplayer game infrastructure has requirements that have no equivalent in standard web application development. Real-time state synchronization between players requires low-latency data paths that are fundamentally different from the request-response patterns of web services. Matchmaking systems must balance player skill, latency, and session availability across a dynamic pool that changes with every match completion. Anti-cheat infrastructure must detect manipulation without introducing latency that degrades the experience for legitimate players. Each of these is a specialized engineering problem with established solution patterns and specific implementation tradeoffs.",
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "Monetization systems — in-app purchase processing, subscription management, and platform-specific payment flows — must handle the specific requirements of each platform the game is distributed on. Apple and Google have distinct review requirements, payment processing constraints, and subscription management behaviors. Real-money-to-virtual-currency conversions carry regulatory implications in some markets. INX designs monetization infrastructure that addresses these requirements explicitly rather than discovering them during platform review.",
        ],
      },
      {
    ...
    capabilities: [
      ...
      "Cloud-hosted game backend deployment",
    ],
    faqs: [
      {
      ...
      },
      {
        ...
        answer:
          "Real-time multiplayer at scale requires architecture decisions that are different from standard web application scaling. Game server allocation must respond to player demand dynamically — standing up new game server instances in response to matchmaking demand and releasing them when sessions complete. Session state must be managed to handle player disconnections and reconnections gracefully. The network layer must minimize latency between players, which typically means deploying game server infrastructure in multiple regions close to the player population. INX designs multiplayer infrastructure with realistic peak load assumptions and load-tests against those assumptions before launch.",
      },
      {
