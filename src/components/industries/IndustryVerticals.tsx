...
const industries: Industry[] = [
  ...
  },
  {
    ...
    complexity:
      "Route assignment as a reactive, manual process is a vehicle utilization problem that compounds across every operational day. The efficiency lost to unoptimized routing is invisible in day-to-day operations and visible only in aggregate financial performance. A driver mobile application that degrades under poor connectivity - which is the operating condition for last-mile logistics, not an edge case - is not a mobile application: it is a partially functional tool with unpredictable failure modes at the moments of highest operational importance.",
    engineering:
      "WebSocket-based real-time geolocation broadcast with sub-5-second update latency to dispatch command dashboards. Offline-tolerant React Native driver applications with local state persistence and server synchronization on reconnection - no data loss under connectivity interruption. Constraint-based route optimization engines accounting for vehicle capacity, delivery time windows, and zone density, implemented without dependency on black-box external APIs with uncontrolled cost and reliability profiles. Parallel operation phases with legacy systems before full cutover to eliminate hard-migration risk. SMS notification pipelines triggered at dispatch, en-route, and delivery completion events.",
    requirements: [
      "Real-Time Tracking",
      "Offline Mobile",
      "Route Optimization",
  ...
  },
  {
    ...
    challenges:
      "Inventory state diverging between physical and digital channels is a customer-facing failure that compounds with time and location count. Each hour of latency between a physical stock event and a digital catalog update is a window for overselling, customer disappointment, and service overhead. High-throughput transaction processing during peak periods - promotional events, seasonal load - requires tested throughput assumptions validated against realistic concurrency, not optimistic ones derived from average traffic.",
    complexity:
    ...
    positioning:
      "INX engineers the inventory, payment, and fulfillment infrastructure that retail operations require to operate at scale without the compounding inaccuracy produced by disconnected systems.",
  },
  {
    index: "04",
  ...
  },
  {
    ...
    engineering:
      "Structured output enforcement at the inference layer using validated JSON schemas - eliminating ambiguous free-form outputs that require downstream parsing with unpredictable failure modes. Confidence-threshold routing between automated processing and human review queues, with every routing decision logged with its model input, output, and score for audit reconstruction. Stateless containerized inference workloads that scale independently of application logic and can be updated, rolled back, or replaced without application-layer changes. Model performance monitoring to detect output drift before it affects production decisions - not after a failure surfaces the degradation. Full audit trail from document or data ingestion through every processing stage to final disposition.",
    requirements: [
      "Structured Output",
