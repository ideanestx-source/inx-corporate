...
export default function LabTechnology({ technologies }: Props) {
        ...
        <Reveal delay={0.05}>
          <div className="flex flex-wrap gap-2.5">
            {technologies.map((t) => (
              <span
                key={t}
                className="font-mono text-[12px] text-white/60 border border-white/[0.09] rounded-[2px] px-3.5 py-1.5"
              >
                {t}
              </span>
            ))}
